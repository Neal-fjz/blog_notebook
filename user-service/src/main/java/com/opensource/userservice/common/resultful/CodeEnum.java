 ...
 * 409	由于和被请求的资源的当前状态之间存在冲突，请求无法完成。这个代码只允许用在这样的情况下才能使用：用户能够被……认为解决冲突，并且会重新提交新的请求。该响应应当包含足够的信息以便用户发现冲突的源头。 　　冲突通常发生于对 PUT 请求的处理中。例如，在采用版本检查的环境下，某次 PUT 提交的对特定资源的修改请求所附带的版本信息与之前的某个（第三方）请求向冲突，那么此时服务器就应该返回一个409错误，告知用户请求无法完成。此时，响应实体中很可能会包含两个冲突版本之间的差异比较，以便用户重新提交归并以后的新版本。
...
public enum CodeEnum {
    ...
    // 2 开头的状态码
    CONTINUE(100, "可以继续发送请求"),
    SWITCH_PROTOCOLS(101, "请切换网络通讯协议"),
    OK(200, "操作成功"),
    CREATED(201, "创建成功"),
    ACCEPTED(202, "已接收请求 正在处理中..."),
    NO_CONTENT(204, "操作成功 不刷新页面"),
    RESET_CONTENT(205, "操作成功 重置内容"),
    PARTIAL_CONTENT(206, "成功获取数据"),

    // 3 开头的状态码
    TEMPORARY_REDIRECT(307, "请求临时重定向"),

    // 4 开头的状态码
    BAD_REQUEST(400, "请求参数有误"),
    NO_TOKEN(401, "未登录 请先登录"),
    FORBIDDEN(403, "权限不足 操作失败"),
    NOT_FOUND(404, "请求失败 无服务-404"),
    METHOD_NOT_ALLOWED(405, "PUT DELETE 请求不被允许"),
    DATA_ALREADY_EXIST(406, "数据已存在"),
    DATABASE_OPERATION_FAILURE(407,"数据操作失败"),
    REQUEST_TIME_OUT(408, "请求超时"),
    PRECONDITION_FAILED(412, "请求不合法"),
    REQUEST_ENTITY_TOO_LARGE(413, "请求数据过大"),
    REQUEST_URI_TOO_LARGE(414, "请求网址过长 无法处理"),
    UNSUPPORTED_MEDIA_TYPE(415, "请求实体无法处理"),
    TOO_MUCH_CONNECTIONS(422, "请求过于频繁 稍后再试"),

    // 5 开头的状态码
    INTERNAL_SERVER_ERROR(500, "服务器开小差 请稍后重试"),
    NOT_IMPLEMENTED(501, "请求不被支持"),
    BAD_GATEWAY(502, "抱歉 服务器开小差..."),

    // 6 自定义操作成功的提示
    Login_success(200,"登录成功"),
    Logout_success(200,"退出登录"),

    SAVE_SUCCESS(200,"创建成功"),
    UPDATE_SUCCESS(200,"更改成功"),
    DELETE_ONE_SUCCESS(200,"删除成功"),
    DELETE_LOT_SUCCESS(200,"批量删除成功"),

    // 7 自定义操作失败的提示
    LOGIN_PWD_ERROR(402,"密码错误"),
    LOGIN_NON_USERNAME_PWD(402,"请输入账号和密码"),
    ACCOUNT_NOT_EXIST(402,"账号不存在"),

    SAVE_FAIL(407,"创建失败"),
    UPDATE_FAIL(407,"更改失败"),
    DELETE_ONE_FAIL(407,"删除失败"),
    DELETE_LOT_FAIL(407,"批量删除失败"),
    NON_MEMBER(409,"请至少选择一个"),
